...
exports.run = (client, message, args) => {
  ...
  if (reason.length < 1) return message.reply('Onun İsmini Söyle Ben Cismini Bulurum, Sebebin Nedir ? .');
  if (message.mentions.users.size < 1) return message.reply('Kimi sunucudan atacağını yazmalısın.').catch(console.error);

  if (!message.guild.member(user).kickable) return message.reply('O Horoz Beni Öldürür Abi LÜTFEN İsteme Bunu Benden.');
